...
| GitHub API write fails — `403 Forbidden` | One of three causes — check in order: (1) **PAT pending org approval** — fine-grained PATs against `cybereason-labs` require IT approval; verify status at GitHub user settings → Developer Settings → Fine-Grained Tokens → `DS-Brain` (status must be `Active`, not `Pending`). See `references/setup.md` Step 1a. (2) PAT lacks `Contents: Read and write` on this repo. (3) User isn't a collaborator on `cybereason-labs/ds-brain`. Show the exact response and point at the matching section. |
